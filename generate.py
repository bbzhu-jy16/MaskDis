    ...
    with torch.no_grad():
        for i in range(len(g_list)):
            ...
            for j in tqdm(range(1000)):
                ...
                sample, _ = g_test([sample_z], truncation=args.truncation, truncation_latent=mean_latent, input_is_latent=False, randomize_noise=False, return_feats=False)
                utils.save_image(
                    sample,
                    f"%s/{str(j).zfill(6)}.png" % (args.imsave_path),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_sample', type=int, default=1, help='number of fake images to be sampled')
    parser.add_argument('--n_steps', type=int, default=1000, help="determines the granualarity of interpolation")
    parser.add_argument('--truncation', type=float, default=1)
    parser.add_argument('--truncation_mean', type=int, default=4096)
    parser.add_argument('--ckpt_source', type=str, default=None)
    parser.add_argument('--ckpt_target', type=str, default=None, required=True)
    parser.add_argument('--imsave_path', type=str, default=None, required=True)
