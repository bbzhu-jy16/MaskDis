...
class Patch_Discriminator(nn.Module):
    ...
    def forward(self, inp, ind = None, extra = None, flag = None, p_ind = None, real=False, return_feats=False):
        feat = []
        for i in range(len(self.convs)):
            if i == 0:
                inp = self.convs[i](inp)
        ...
        out_part[:,index]=0
         ...
         # (4,1)
        out = self.extra_linear(out_part)
...
class Noise_Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1,3,3,1]):
        ...
        for i in range(log_size, 2, -1):
            ...
            convs.append(ResBlock(in_channel, out_channel, blur_kernel))
            noise_injections.append(NoiseInjection())

            in_channel = out_channel

        self.convs = nn.Sequential(*convs)
        self.noise_injections = nn.Sequential(*noise_injections)
        self.stddev_group = 4
        self.stddev_feat = 1
        ...
        for i in range(len(self.convs)):
            if i == 0:
                ...
                if (flag > 0) and (temp2.shape[1] == 512) and (temp2.shape[2] == 32 or temp2.shape[2] == 16):
                    feat.append(temp2)
                inp = self.convs[i](inp)
                inp = self.noise_injections[i](inp)
                if (flag > 0) and len(feat) == 4:
                    # We use 4 possible intermediate feature maps to be used for patch-based adversarial loss. Any one of them is selected randomly during training.
                    inp = extra(feat[p_ind], p_ind)
                    #if return_feats:
