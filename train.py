...
def train(args, loader, generator, discriminator, extra, g_optim, d_optim, e_optim, g_ema, device, g_source, d_source):
    ...
    imsave_path2 = 'cluster_centers/'+args.dataset+'/generate_examples'
    ...
    sample_z_init = torch.randn(1000, args.latent, device=device)

    for j in tqdm(range(1000)):
        #sample_z_init = torch.randn(1, args.latent, device=device)          
        sample, _ = g_source([sample_z_init[j:j+1]], truncation=1, truncation_latent=None, input_is_latent=False, randomize_noise=False)
        utils.save_image(
            sample,
            f"%s/{str(j).zfill(6)}.png" % (imsave_path2),
            nrow=int(1 ** 0.5),
            normalize=True,
            range=(-1, 1),
        )

    os.system("python feat_cluster.py --baseline ours --dataset " + args.dataset)
...
#---------------------------------------------------------------------------------------------------
# discriminator cross-domain consistency loss      
        with torch.set_grad_enabled(False):
            z = torch.randn(args.feat_const_batch, args.latent, device=device)
            #feat_ind = numpy.random.randint(1, g_source.module.n_latent, size=args.feat_const_batch) #full dataset
            feat_ind = numpy.random.randint(0, 4, size=args.feat_const_batch) # 10-shot
            #feat_ind = numpy.random.randint(1, 11, size=args.feat_const_batch)
            
            # computing source distances in discriminators
            source_sample, _ = g_source([z], return_feats=False)
            _, feat_source = d_source(source_sample, extra=extra, flag=which, p_ind=np.random.randint(lowp, highp), return_feats=True)
            #print(len(feat_source))
        ...
        target_sample, _ = generator([z], return_feats=True)
        _, feat_target = discriminator(target_sample, extra=extra, flag=which, p_ind=np.random.randint(lowp,highp), return_feats=True)
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if get_rank() == 0:
            ...
            if (i % args.eval_freq == 0) and (i > 0):
                for j in tqdm(range(1000)):
                    #sample_z_example = torch.randn(1, args.latent, device=device)          
                    sample, _ = g_ema([sample_z_init[j:j+1]], truncation=1, truncation_latent=None, input_is_latent=False, randomize_noise=False)
                    utils.save_image(
                        sample,
                        f"%s/{str(j).zfill(6)}.png" % (imsave_path2),
                        nrow=int(1 ** 0.5),
